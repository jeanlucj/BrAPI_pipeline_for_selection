...
| `STUDY_NAMES` | **If not NULL**, analyze exactly these trials (by `studyName`) and ignore the radius search. NULL = use the radius search. |
| `STUDY_TYPES` | Vector of allowable study types analyzed together (default `c("phenotyping_trial")`). |
...
| `PEDIGREE_DF` | EM weight given to the pedigree matrix. |
| `BGLR_MODEL`, `BGLR_NITER`, `BGLR_BURNIN`, `SEED` | Genomic-prediction (BGLR) settings. |
| `MAX_MISSING`, `MIN_MAF` | Marker QC thresholds. |

### Two ways to choose trials
- **Geographic (default):** leave `STUDY_NAMES <- NULL`; trials are found within
  `RADIUS_KM` of the center point, of any type in `STUDY_TYPES`, grown in `YEARS`.
- **Explicit:** set `STUDY_NAMES <- c("Trial_A", "Trial_B", ...)`; exactly those
  trials are analyzed (type/year/radius ignored). Names not found on the server
...
trials <- find_ny_trials(conn)                          # 2. select trials
pheno  <- get_phenotypes(conn, trials$studyDbId)        # 3. phenotypes + design
geno   <- find_and_get_genotypes(conn, pheno$accessions)# 4. combined GRM
blues  <- stage1_blues(pheno$pheno)                     # 5. per-trial BLUEs
