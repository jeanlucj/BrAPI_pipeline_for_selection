...
#
# Wishart-EM covariance combiner (Akdemir), copied verbatim from
# ../T3Predictathon2026/scripts/Analysis_Claude/em_covariance_combiner.R so this
# project is self-contained. Trimmed to the functions exercised when
# compute_sampling_cov = FALSE (sampling-covariance routines omitted). Sourced by
# code/04_find_genotyping.R to combine per-protocol (and pedigree) relationship
# matrices into one combined GRM.
#
# Index convention: var_indices are the 1-based positions of each partial
# matrix's accessions within the combined accession ordering (i.e.
# match(colnames, combined_names)). n_vars then becomes max(index) + 1, which
# leaves a phantom variable at psi position 1 that is always "missing"; callers
# drop it with result$psi[-1, -1].
